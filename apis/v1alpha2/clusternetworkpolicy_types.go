...
type ClusterNetworkPolicyIngressRule struct {
	...
	// protocol-specific attributes such as the port. If match is empty, then
	// the rule will match all traffic.
	//
	// +optional
	Protocols *ClusterNetworkPolicyProtocols `json:"protocols,omitempty"`
}
...
type ClusterNetworkPolicyEgressRule struct {
	...
	// Protocols allows for more fine-grain matching of traffic on
	// protocol-specific attributes such as the port. If match is empty, then
	// this rule will match all traffic.
	//
	// +optional +kubebuilder:validation:MinItems=0
	// +kubebuilder:validation:MaxItems=25
	Protocols *ClusterNetworkPolicyProtocols `json:"protocols,omitempty"`
}
...
// ClusterNetworkPolicyProtocols describes additional protocol-specific match rules.
type ClusterNetworkPolicyProtocols struct {
	...
	// do not have named ports.
	//
	// TODO: what is the behavior if named port reference is invalid?
	DestinationNamedPort []ClusterNetworkPolicyNamedPort `json:"destinationNamedPort,omitempty"`
}

// ClusterNetworkPolicyMatchTCP are TCP attributes to be matched.
type ClusterNetworkPolicyMatchTCP struct {
	DestinationPort *ClusterNetworkPolicyPort `json:"destinationPort,omitempty"`
}
...
// ClusterNetworkPolicyNamedPort matches the protocol and port given by the
// PodSpec.Containers.Ports.Name.
type ClusterNetworkPolicyNamedPort struct {
	// Names are the named ports to be selected.
	Name *string `json:"name,omitempty"`
}

// ClusterNetworkPolicyPort matches on port number.
//
// +kubebuilder:validation:MaxProperties=1
// +kubebuilder:validation:MinProperties=1
type ClusterNetworkPolicyPort struct {
	...
	// Range is a range of ports.
	Range *ClusterNetworkPolicyMatchPortRange `json:"range,omitempty"`
}

// ClusterNetworkPolicyMatchTCP are TCP attributes to be matched. Port ranges
// are inclusive of their endpoints.
//
// Example:
//
//	{ start: 80, end: 100 } will select the ports 80 to 100.
