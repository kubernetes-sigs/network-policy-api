...
type ClusterNetworkPolicySpec struct {
	...
	// While tiers can only be set on the ClusterNetworkPolicy, they ara some virtual tiers
	// that exist in the cluster: NetworkPolicy tier contains all NetworkPolicies in the cluster
	// and DefaultTier which is the default cluster policy that allows all pods to communicate with each other.
	//
	...
	// Every ClusterNetworkPolicy withing the same tier should have a unique priority value;
