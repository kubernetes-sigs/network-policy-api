...
type AdminNetworkPolicyEgressPeer struct {
    <snipped>
    // FQDN provides a way to specify domain names as peers.
    //
    // FQDN is only supported for ALLOW rules. In order to control access, FQDN
    // ALLOW rules should be used with a lower priority egress deny -- this
    // allows the admin to maintain an explicit "allowlist" of reachable
   // domains.
    //
    // Support: Extended
    //
...
This example, includes the DENY rule in the same ANP object:
```yaml
apiVersion: policy.networking.k8s.io/v1alpha1
kind: AdminNetworkPolicy
metadata:
...
1. DNS record querying and lifetimes:
   ...
   *  When the TTL for a DNS record expires, the implementor should stop
      ...
      connections). 
1. Targeting in-cluster endpoints with FQDN selector:
   *  ✅︎ Supported:
