...
func init() {
	ConformanceTests = append(ConformanceTests,
		CNPBaselineTierPortRange,
	)
}

var CNPBaselineTierPortRange = suite.ConformanceTest{
	ShortName:   "CNPBaselineTierPortRange",
	Description: "Tests support for combined ingress and egress port range rules across protocols in ClusterNetworkPolicy on Baseline Tier",
	Features: []suite.SupportedFeature{
		suite.SupportClusterNetworkPolicy,
	},
	Manifests: []string{"base/baseline_tier/standard-port-range-rules-combined.yaml"},
	Test: func(t *testing.T, s *suite.ConformanceTestSuite) {

		t.Run("Should support egress port range rules across different protocols", func(t *testing.T) {
			// This test uses `port-range-rules` baseline CNP where Subject is gryffindor.
