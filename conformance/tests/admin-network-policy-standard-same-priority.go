...
var CNPAdminTierSamePriority = suite.ConformanceTest{
	...
	Description: "Tests that ClusterNetworkPolicies in the Admin tier with the same priority are all applied, both when they share a subject with disjoint rules and when they reference the same peer from different subjects, in both ingress and egress directions",
	Features: []suite.SupportedFeature{
		suite.SupportClusterNetworkPolicy,
	},
	Manifests: []string{"base/admin_tier/standard-same-priority.yaml"},
	Test: func(t *testing.T, s *suite.ConformanceTestSuite) {
		...
		//   same-priority-gryffindor-slytherin: subject gryffindor, denies slytherin at port 80
		//   same-priority-gryffindor-hufflepuff: subject gryffindor, denies hufflepuff at port 80
		//   same-priority-hufflepuff-slytherin: subject hufflepuff, denies slytherin at port 8080
		...
		slytherinServer := kubernetes.GetPod(t, s.Client, "network-policy-conformance-slytherin", "draco-malfoy-0", s.TimeoutConfig.GetTimeout)
		...
		ravenclawServer := kubernetes.GetPod(t, s.Client, "network-policy-conformance-ravenclaw", "luna-lovegood-0", s.TimeoutConfig.GetTimeout)
		...
		t.Run("Should deny traffic in both directions for each same-priority policy", func(t *testing.T) {
			// same-priority-gryffindor-slytherin ingress: deny from slytherin to gryffindor at port 80
			...
			// same-priority-gryffindor-slytherin egress: deny from gryffindor to slytherin at port 80
			...
			// same-priority-gryffindor-hufflepuff ingress: deny from hufflepuff to gryffindor at port 80
			kubernetes.PokeServer(t, s.ClientSet, &s.KubeConfig, "network-policy-conformance-hufflepuff", "cedric-diggory-0", "tcp",
				gryffindorServer.Status.PodIP, int32(80), s.TimeoutConfig, false)
			// same-priority-gryffindor-hufflepuff egress: deny from gryffindor to hufflepuff at port 80
			...
			// same-priority-hufflepuff-slytherin ingress: deny from slytherin to hufflepuff at port 8080
			...
			// same-priority-hufflepuff-slytherin egress: deny from hufflepuff to slytherin at port 8080
		...
		t.Run("Should not apply one same-priority policy's rules to another policy's subject", func(t *testing.T) {
			// same-priority-hufflepuff-slytherin's port-8080 rules must not bleed
			// onto subject gryffindor: slytherin to gryffindor at port 8080 matches
			// no policy and must stay allowed
			kubernetes.PokeServer(t, s.ClientSet, &s.KubeConfig, "network-policy-conformance-slytherin", "draco-malfoy-0", "tcp",
			...
			// same-priority-gryffindor-slytherin's port-80 rules must not bleed
			// onto subject hufflepuff: slytherin to hufflepuff at port 80 matches
			// no policy and must stay allowed
			kubernetes.PokeServer(t, s.ClientSet, &s.KubeConfig, "network-policy-conformance-slytherin", "draco-malfoy-1", "tcp",
